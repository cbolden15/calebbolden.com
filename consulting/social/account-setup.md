...
**1. Handle availability.** Before creating anything, check whether `@calebbolden` is available on LinkedIn, YouTube, Instagram, and Facebook. Check all four before touching any of them. If it is taken on even one platform, use `@calebboldenai` on all four instead, not just the one where the preferred handle failed. A consistent handle across all four platforms matters more than winning the preferred handle on three of them and settling for a mismatch on the fourth. Display name is `Caleb Bolden` on every platform regardless of which handle wins.

**2. LinkedIn (reposition, do not create).** LinkedIn permits one profile per person. A second profile gets merged into the first or banned outright, and appealing a ban takes weeks. There is no "create a new LinkedIn for the consulting business" option here: the existing personal profile is the account. Rewrite the headline, About, and Featured sections to speak to the operators audience: operations leaders at SMBs large enough to have a real coordination problem, not owner-operators. Add "Vora Technologies Consulting" under the Experience section. This is the only place in this entire setup where the LLC name appears. It does not go on YouTube, Instagram, or Facebook.

**3. YouTube.** Create a Brand Account under the existing Google account, named `Caleb Bolden`. A Brand Account is a separate channel identity layered on the same Google login, not a second Google account, so this carries none of the risk described in steps 2 and 5. This channel holds both the Shorts feed and the longer clinic recordings. Set the handle to whatever step 1 resolved.

**4. Instagram.** Create a new Instagram account. Instagram allows multiple accounts under one login, so a new account here does not put any existing personal Instagram at risk the way a second LinkedIn or Facebook profile would. Set the handle and display name to match steps 1 and 2, and write the bio for the owners audience: plumbers, electricians, salon owners, landscapers.
