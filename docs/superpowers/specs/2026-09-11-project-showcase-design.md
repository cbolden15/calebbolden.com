...
Status: Retained cycle-3 findings applied to this spec and its implementation plan. The last hardening run reviewed earlier bytes and returned fix-first: 0 Critical, 7 Important, 1 Minor groups. All four lens reviews and their finding audits completed; the blind independent reviewer timed out. These corrections have not received a new hardening verdict. Implementation planning remains provisional; application implementation and publication have not started. See Document review below.
...
Each entry has a poster, project name, a 25–45 word summary, a concise contribution line, maturity, and one clear detail link. This release uses no gallery enlarge control; an approved full-size raster may have a separate native link. Do not nest controls inside a whole-card link.
...
| 2 | Primary demonstration | Large media frame, visible demonstration label, short caption, interaction controls, and optional native approved-image link. Gallery work is deferred. Appears before stack details. |
...
Catalog-entry publication and rich-case-study publication are separate. Derive Vora's route state from its optional rich body and that body's publication flag; do not introduce another mutable cutover flag. The existing explicit `/work/vora` route owns resolution in every state:

| Vora state | Production response and public navigation | Transition and rollback |
|---|---|---|
| Legacy | No rich body exists. Render the retained legacy body at the existing URL, with the catalog's qualified public summary and maturity. Keep its catalog, related-link, metadata, and sitemap destination. | Adding a draft rich body moves to the next state without changing the public destination. |
| Rich draft with legacy fallback | A rich body exists but is draft. Production still renders only the legacy body; draft narrative, fixtures, and assets do not enter public responses or client bundles. Local development may show the labeled rich draft. | Publish the rich body only with complete evidence and passing route/asset gates. Removing it returns to Legacy. |
| Rich published | Render the approved rich body at the same URL. Public catalog, metadata, related links, and sitemap resolve this view together from one server-side selector. | Unpublish or remove the rich body to restore the retained legacy view. Keep that fallback available throughout this release. |

Validate each state and both rollback transitions before release. Resolve the body, links, and metadata from the same record revision in one build; do not switch routing through a second deployment or separate flag. Newly added flagship routes have no legacy fallback: drafts return production 404 and become visible only when their rich body and evidence are published. A failed publication check fails the build rather than silently switching views. Returning Vora to its legacy view also removes rich-only assets from the production asset set unless another published surface still references them.
...
When JavaScript is unavailable, render the first frame plus a complete ordered text walkthrough, including every scenario and terminal branch. Required showcase content must not inherit the existing Reveal component's initially invisible state. Hide enhancement-only controls until their module and hydration are ready. The server-rendered demonstration shell owns the static content and a native link to reload the current page. That link is outside the lazy demo module and requires no handler or hydration, so recovery remains usable if its chunk fails to load. A runtime error boundary may hide a failed island; it must not remove the shell or its recovery link. No custom retry state machine, module-retry service, or retry control inside the failed module is required.

Media failure retains the caption, text walkthrough, and native reload link. An optional open-image link points only to a manifest-approved inert raster asset under the media policy in section 13; it is ordinary navigation, not an arbitrary source URL. A failed video never removes the case study's explanation. Browser checks must block the demo chunk and media requests separately and verify that static content and native recovery remain usable with no dead enhancement controls.
...
| Media | Default overview poster 16:10; preserve the source ratio when cropping would hide important UI. Use a legible crop and, when needed, a native link to its approved full-size raster. A crop always has a caption describing the selected region. |
...
Define one canonical server-side catalog in a dedicated public-content module. Homepage, Work, How I build, related-project links, and detail pages use typed projections from it rather than maintaining separate status/summary arrays. A Work filter receives only a public card DTO: identity, category, summary, contribution, badge, approved poster, order, and validated destination. Related links and homepage proof receive their smaller projections; a detail page alone receives its narrative and selected example. Fixture data remains in server-only authored modules; client islands import only pure state logic and receive the selected public fixture through projected props. Client modules must not import the canonical catalog, project records, or fixture data modules, including their own draft fixture. A production 404 alone does not prevent a statically imported fixture from appearing in emitted public chunks. Explicit field projection applies recursively; do not spread whole records into client props.

Use a discriminated union for flagship and retained legacy records. A flagship has the qualified maturity enum and an optional rich `caseStudy` body with its own draft/published state. That body requires Story, Evidence, and one supported Interaction when published. A legacy secondary record has its existing availability/status and optional legacy destination; it requires neither a rich body nor a flagship demo/fixture. ChapterHQ and the site assistant retain their existing pages and media under this branch. Open source remains a separate collection. Homepage-only work may omit a detail route and must not acquire a fabricated link.

| Field group | Required values |
|---|---|
| Identity | Common identity fields: stable slug, name, category, summary, contribution statement, and related slugs. A published Work/case-study placement requires a known resolvable detail route, backed by an approved rich body or retained legacy page. Draft and homepage-only records may omit it. A slug alone never creates a route. |
| Publication | Catalog-entry draft/published state, placements (homepage proof, Work catalog, case study, How I build), display order, qualified flagship maturity or separate legacy status, public checked date, and optional reviewed product/source/documentation URLs. A flagship's optional rich body has a separate publication state; Vora follows section 5's fallback rules. |
| Story | Required in a published rich body: problem, workflow introduction, engineering decisions, limits, and contribution credits. Retained legacy records use their existing body contract. |
| Evidence | Required in a published rich body: approved snapshot ID, media kind, poster/source, caption, alt text, dimensions, transcript or walkthrough, and public “About this example” explanation derived from that snapshot. Prism also requires version, public source tag/commit, derivation type, and checked date. Legacy media retains its existing contract. |
| Interaction | Required in a published rich body: one of the five supported demo kinds, its approved snapshot/fixture ID, and display copy. Legacy records omit this group. How I build placements also require responsibility label, order, and a compact static selection/caption referencing the approved example. No arbitrary scripts or private service configuration. |

Validate the complete public graph at build time: unique slugs, legal placements, existing explicit route files, supported resolved views, resolvable published related destinations, and matching metadata/sitemap selectors. Publishing a flagship without its required detail route or publishing a new rich-draft-only route fails validation. The type checks and runtime schema must reject missing rich evidence while accepting unchanged secondary records without demo fields. Related draft destinations are omitted by the public projection, not converted into guessed URLs.
...
Store each approved example in a reviewed evidence manifest, separate from the bytes it attests. Its snapshot ID binds the exact fixture source files and media files to SHA-256 digests, derivation (captured or illustrative/contract-derived), checked date, disclosure, and allowed public fields. Hash the full bytes of each listed fixture source or serialized fixture file; do not store a self-hash inside that file. Source version/revision is additional context, not a substitute for the digest of the actual public sample. Interactions reference the snapshot ID, and displays obtain their checked date and provenance from the same manifest entry.

Any changed fixture or asset invalidates the existing attestation. Production publication requires complete entries for the resolved public examples, and every recorded attestation fails validation on a digest mismatch until the replacement bytes and disclosure have been reviewed and a new snapshot revision approved. Never auto-refresh digests during the production build. Include a negative test that changes fixture content while leaving its old date/disclosure/manifest unchanged; schema-valid replacement content must still fail. This contract applies to all five published examples, including explicitly illustrative fixtures.

Local-only synthetic development previews validate the fixture's strict public-safe content schema on the server but do not require production snapshot approval. Use a distinct local-synthetic provenance state, omit missing version/revision/checked-date fields, and show an explicit unapproved-for-publication label and media placeholders. Never label this state recorded or approved. The published rich-body variant requires the complete evidence contract; a draft variant may remain incomplete. Test that a draft renders locally with an empty approval manifest while the same record returns production 404 or Vora's legacy fallback. This adds no deployed preview switch, does not waive checks on existing manifest attestations, and does not allow fixture imports into client modules.
...
Only approved release assets live under `public/work/<slug>/` within this website; this path is a future implementation target. Unreviewed captures and source material stay outside the website repository and its build inputs. Local draft development uses purpose-written synthetic fixtures and explicit placeholders; a draft page's 404 is not an asset access control. A file copied into `public/` can be requested directly regardless of its case-study route state.

Before every production build, derive the required showcase asset set from currently published resolved views and their reviewed evidence manifest entries. Compare that set exactly with the showcase public tree: each file has an approved relative path, digest, format/MIME, and at least one published surface that uses it. Every required file exists, and every file in that tree is required; reject extra, stale, changed, or draft-only files. An approved snapshot belonging only to a draft does not make its assets public. Copy only approved files into that tree during preparation. The build validates and fails closed; it does not approve or silently remove content. Paths cannot escape the showcase tree, and symlinks are rejected. This gate does not reclassify unrelated existing public assets or change preview access rules. Verify production direct asset URLs and serialized responses/client bundles using synthetic draft sentinels, as well as checking page 404s. Draft fixture modules must not enter shared client imports.

Allow only inert raster images (PNG, JPEG, WebP, AVIF) for showcase posters and open-image destinations, and reviewed MP4/WebM for video sources. Validate extension, decoded file/signature, and expected MIME together; a renamed HTML file is not an image. Check the served Content-Type in the production browser verification. Reject HTML, SVG, XML, arbitrary JSON downloads, and other active/document formats from the showcase asset tree and open-image links. Render diagrams as trusted authored components or approved raster output; fixture JSON stays behind typed projections, not a raw public download. Hosting active documents on an isolated origin would require a separate reviewed delivery design and is outside this release.

Use descriptive filenames without private paths, customer IDs, tokens, or internal hostnames. Strip unnecessary image metadata. Put the evidence label in HTML, not only inside an image.

The initial inventory contains no approved gallery set. Defer the gallery component, dialog controls, and gallery lifecycle acceptance for this release. Use the poster, readable crop, walkthrough, and optional native approved-image link. A later gallery task must identify the reviewed images it serves before adding enlargement controls or gallery requirements; it is not part of the shared-foundation completion gate.
...
|---|---|
| Keyboard and focus | Every filter, step, scenario, disclosure, and media control is reachable and labeled. Ordered-sequence Previous/Next disable at the ends; other demos follow their named transitions. Gallery focus trapping, Escape, and opener-focus restoration apply only to a separately scoped future gallery. |
...
Planning budgets: target each optimized poster at 250KB or less and additional compressed JavaScript for the active case-study demonstration at 100KB or less. Measure before/after during implementation. If evidence is illegible within a media budget, use a focused crop and optional link to an approved larger raster. A budget exception must state what needs the bytes; do not quietly replace useful evidence with unreadable compression. Inspect the Work client payload separately: it contains card projections, not every narrative, transcript, or demo fixture.
...
|---|---|
| A1: Navigation and content | Five flagship destinations resolve; every migration-map route remains reachable; Vora's three states and rollback preserve its URL, resolved body, metadata, and links. Filters return the exact mapped entries and counts (7/3/4); Open source remains outside counts; category deep links and back/forward work; featured entries are not duplicated; no private-source button, new draft-only route, or link to an unresolvable related destination is published. Published case-study placements require valid routes; secondary records work without flagship fields. Existing client-work `/results` and audit/readiness paths remain available. |
...
| A3: Evidence integrity | Every flagship has a truthful contribution, maturity, media label, and checked date bound to the approved fixture/asset digests. Mutating sample bytes without a new reviewed manifest revision fails validation. Prism records version, public source revision, and captured/contract-derived provenance. Config-gated behavior is qualified. No fixture is described as current operational data, and no diagram invents cross-project integration. |
| A4: Presentation and access | Check desktop, narrow available width with chat open, mobile, keyboard, zoom, reduced motion, failed media, failed demo chunks, and no-JavaScript fallback. Verify readable static content, native reload/open-image links independent of failed enhancements, absence of dead controls, and the existing contact destination. Gallery lifecycle is deferred with the gallery subsystem. |
| A5: Build and boundaries | Project tests/build and the showcase asset gate pass; public schema/serialization tests reject private identifiers and imports. Exact approved file paths/digests/formats match the public tree; draft sentinels are absent from direct URLs and client bundles; active or mislabeled media fail validation. Work client props contain only card fields, and each detail loads only its selected fixture. Any Vora action capture has documented isolation and sandbox/test or mocked delivery. Browser network inspection shows no demo call to an AI provider, messaging endpoint, private host, or process launcher. Served media MIME, dimensions/loading, and bundle budgets are checked. |
...
|---|---|---|
| 1. Shared foundation | Typed catalog/projections, case-study sections, approved evidence manifest and asset gate, responsive media, static shell with native recovery. | Existing project pages and Vora's fallback states work; publication and exact-byte evidence boundaries are established. No gallery subsystem is required. |
...
The unmodified cycle-1 report is `/Users/calebbolden/Projects/consulting/calebbolden.com/docs/superpowers/specs/2026-09-11-project-showcase-design.md.hardening.cycle-1.md`; its dispositions and the incomplete second cycle remain archived beside it. After runtime repair and explicit approval, cycle 3 reviewed SHA-256 `3d5be73004e60931083ee486f86645335292addd8f67735bc0ca3d18f348d553` and returned fix-first: 0 Critical, 7 Important, 1 Minor groups. All four primary reviews and their independent finding audits completed. The blind independent reviewer timed out, so `single_engine: true`; the completed counts do not establish the absence of Critical findings from that missing seat.

The primary report at `/Users/calebbolden/Projects/consulting/calebbolden.com/docs/superpowers/specs/2026-09-11-project-showcase-design.md.hardening.md` preserves the verdict for cycle 3's reviewed bytes; it is not an attestation of this later revision. User-authorized corrections address all eight retained groups, including the legacy-schema and native-recovery members merged under representative findings. The refuted analytics-workstream and forced-word-count findings remain refuted. Correction mapping and actual checks are recorded in `/Users/calebbolden/Projects/consulting/calebbolden.com/docs/plans/2026-09-11-project-showcase-verification.md`. No new hardening run occurred. The implementation plan remains provisional; application tasks and their tests have not started.
