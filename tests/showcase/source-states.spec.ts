...
test('fresh isolated source candidate has explicit HTTP body metadata catalog sitemap asset and all-chunk parity', async ({ page }, info) => {
  ...
  for (const route of ['/', '/work', '/how-i-build', '/work/vora', '/work/prism', '/work/agent-team', '/work/agent-config', '/work/control-center', '/sitemap.xml']) {
    ...
    if(route==='/work/vora') {
      ...
      if(!richVora) { await expect(page.getByText('what it is',{exact:true})).toBeVisible(); await expect(page.locator('meta[name="description"]')).toHaveAttribute('content',/AI CRM|AI-native|AI-powered/); }
    }
    ...
    if (route.startsWith('/work/') && route !== '/work') {
      ...
      else { expect(selected.view.kind).toBe('not-found'); expect(draft).toBe(true); }
    }
