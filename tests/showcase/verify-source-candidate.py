...
import pathlib, json, os, subprocess, sys, tarfile, shutil
...
# Test-only shutdown transport may advance after the measured application source commit.
shutil.copyfile(root/'playwright.config.ts',source/'playwright.config.ts')
shutil.copyfile(root/'playwright.config.ts',evidence/'playwright.config.ts')
...
if expected['state']=='four-new-drafts':
    ...
    env['SHOWCASE_SERVER']='development'
    env['SHOWCASE_SOURCE_DRAFTS']='four-new-drafts'
    env['SHOWCASE_RESULTS']=str(evidence/'development-browser')
    command=['npx','playwright','test','tests/showcase/draft-development.spec.ts','--project=chromium']
    if env.get('SHOWCASE_DRAFT_DEVELOPMENT_GREP'): command.extend(['--grep',env['SHOWCASE_DRAFT_DEVELOPMENT_GREP']])
    run('development-browser',command)
    (evidence/'development-result.json').write_text(json.dumps({'log':'development-browser.log','exit':0},indent=2))
